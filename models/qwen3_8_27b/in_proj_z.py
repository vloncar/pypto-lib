...
# -----------------------------------------------------------------------------------------------------------
"""`in_proj_z` in A8W8: hidden -> the gated RMSNorm's gate.

    z = x @ W^T            [T, 5120] -> [T, 6144], INT8 both sides, bf16 out

The arithmetic, the golden and the CLI are `a8w8_linear`; this file is the
shape, the tiles, and where the weight and the references come from. `z` is
read once, by the norm at the far end of the block, and nothing between the
two touches it.
"""
import a8w8_linear
...
N = QWEN3_8_27B.value_width                 # H value heads of D
...
def compare_y(t: int, weights: str | None):
    ...
    return a8w8_linear.compare_y(truth, fakequant, "z")
...
if __name__ == "__main__":
    ...
    a8w8_linear.report("in_proj_z", args, result, K, N)
